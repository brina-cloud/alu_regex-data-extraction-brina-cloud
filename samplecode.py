...
def hide_email(email):
    ...
    parts = email.split('@') #split the email into local and domain parts
    local = parts[0]
    domain = parts[1]
    # if the local part is less than or equal to 2 characters, hide all but the first character #
    if len(local) <= 2:
        hidden_local = local[0] + '*' * (len(local) - 1)
    elif len(parts) != 2:
        # if the email format is invalid, return it as is #
        return email
    else:
        # hide all but the first and last characters of the local part #
...
with open('sampleoutput.json', 'w') as json_file:
    ...
    print("\n***** EXTRACTION complete. Check sampleoutput.json for results.  *****\n")
